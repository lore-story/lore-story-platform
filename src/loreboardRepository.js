...
export function normalizeLoreboard(value, now = Date.now()) {
  ...
  return { phases, activePhase: Math.min(Math.max(0, Number(value?.activePhase) || 0), phases.length - 1), assignment: typeof value?.assignment === 'string' && value.assignment.trim() ? value.assignment.trim().slice(0, MAX_ASSIGNMENT_LENGTH) : DEFAULT_LOREBOARD_STATE.assignment, materials, timer: restoreTimer({ getItem: () => JSON.stringify(value?.timer ?? DEFAULT_TIMER) }, now), noiseLevel: ['Leise','Partner','Frei'].includes(value?.noiseLevel) ? value.noiseLevel : DEFAULT_LOREBOARD_STATE.noiseLevel, activeStory: typeof value?.activeStory === 'string' ? value.activeStory : DEFAULT_LOREBOARD_STATE.activeStory, activeWorld, presentationWorldId: typeof value?.presentationWorldId === 'string' ? value.presentationWorldId : activeWorld }
}
