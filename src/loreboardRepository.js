...
export const DEFAULT_LOREBOARD_STATE = Object.freeze({ phases: ['Ankommen', 'Entdecken', 'Vertiefen', 'Teilen'], activePhase: 0, assignment: 'Findet heraus, wie Lebewesen im Wald miteinander verbunden sind.', materials: DEFAULT_MATERIALS, timer: DEFAULT_TIMER, activeStory: 'moosarchiv', activeWorld: 'nebelmark' })
...
export function normalizeLoreboard(value, now = Date.now()) {
  ...
  return { phases, activePhase: Math.min(Math.max(0, Number(value?.activePhase) || 0), phases.length - 1), assignment: typeof value?.assignment === 'string' && value.assignment.trim() ? value.assignment.trim().slice(0, MAX_ASSIGNMENT_LENGTH) : DEFAULT_LOREBOARD_STATE.assignment, materials, timer: restoreTimer({ getItem: () => JSON.stringify(value?.timer ?? DEFAULT_TIMER) }, now), activeStory: typeof value?.activeStory === 'string' ? value.activeStory : DEFAULT_LOREBOARD_STATE.activeStory, activeWorld: typeof value?.activeWorld === 'string' ? value.activeWorld : DEFAULT_LOREBOARD_STATE.activeWorld }
}
