...
export function worldSwitchPlan(story, targetWorldId) {
  const fallback = stories.find(item => item.id === DEFAULT_FREE_STORY_ID)
  return { presentationWorldId: targetWorldId, story: isStoryAvailableInWorld(story, targetWorldId) ? story : fallback, requiresFallback: !isStoryAvailableInWorld(story, targetWorldId) }
}
