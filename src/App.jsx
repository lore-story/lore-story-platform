...
import { isStoryAvailableInWorld, stories, storyCategoryLabel, worlds, worldSwitchPlan } from './data'
...
function LoreboardMode({ fundus, activeStory, setActiveStory, activeWorld, setActiveWorld, setView, repository }) {
  ...
  const novaMessage = world.id === 'astra' ? 'Crew, euer Signal ist stabil. Richtet den Blick auf den Auftrag und arbeitet als Team.' : 'Eine neue Spur ist eingetroffen. Seid aufmerksam und folgt eurem gemeinsamen Auftrag.'
  ...
  return <main className={`loreboard-mode ${configuredTheme ? `world-${world.id}` : ''}`} style={{'--lore-accent':world.colors[0],'--lore-deep':world.colors[1],...themeVariables(configuredTheme)}}>
    ...
    <section className="command-grid">
      ...
      <section className="nova-command">
        <div className="nova-visual"><span className="system-online"><i/> SYSTEM ONLINE</span><div className="nova-core">NOVA</div><small>ASTRA · KOMMANDOSCHNITTSTELLE</small></div>
        <article className="nova-transmission"><header><span>NOVA // LIVE-ÜBERTRAGUNG</span><i aria-label="Signal stabil">▮▮▮▮</i></header><p>{novaMessage}</p></article>
        <article className="board-widget assignment-widget"><div className="widget-title"><PenLine/><span>AKTUELLER AUFTRAG</span><button onClick={openAssignment} aria-label="Auftrag bearbeiten"><PenLine/></button></div><p className="assignment-display">{board.assignment}</p></article>
        <article className="story-launcher"><div><span>AKTIVE MISSION</span><strong>{story.title}</strong><small>{storyCategoryLabel(story)} · {story.subject}</small></div><select aria-label="Aktive Lorestory" value={story.id} onChange={e=>selectStory(e.target.value)}>{available.map(item=><option value={item.id} key={item.id}>{item.title}</option>)}</select><button onClick={()=>setView('mission')} disabled={!getMissionPackage(`lore-${story.worldId || story.world}-${story.id}`)}><CirclePlay/> Mission vorbereiten</button></article>
      </section>
