  ...
  return <main className={`loreboard-mode ${configuredTheme ? `world-${world.id}` : ''}`} style={{'--lore-accent':world.colors[0],'--lore-deep':world.colors[1],...themeVariables(configuredTheme)}}>
    ...
    <section className="command-grid">
      ...
      <section className="assignment-column">
        <article className={`board-widget assignment-widget ${world.id==='astra'?'astra-nova-assignment':''}`}>{world.id==='astra'&&<div className="assignment-media" data-testid="astra-nova-assignment-media" aria-hidden="true" style={{backgroundImage:`url(${ASTRA_LOREBOARD_MEDIA.nova})`}}/>}<div className="widget-title"><PenLine/><span>AKTUELLER AUFTRAG</span><button onClick={openAssignment} aria-label="Auftrag bearbeiten"><PenLine/></button></div><p className="assignment-display">{board.assignment}</p></article>
      </section>
