...
export function userInitials(user) {
  const source = user?.user_metadata?.full_name || user?.user_metadata?.name || user?.email?.split('@')[0] || ''
...
function Platform(props) {
  ...
  const presentationWorldId=resolvePresentationWorldId(selectedStory,props.activeWorld,props.activeWorld)
...
function LoreboardMode({ fundus, activeStory, setActiveStory, activeWorld, setActiveWorld, setView, repository }) {
  ...
  const presentationWorldId=resolvePresentationWorldId(story,board.presentationWorldId,board.activeWorld)
  ...
  return <main className={`loreboard-mode ${configuredTheme ? `world-${world.id}` : ''}`} style={{'--lore-accent':world.colors[0],'--lore-deep':world.colors[1],...themeVariables(configuredTheme)}}>
    ...
    <section className="command-grid">
      ...
      <section className="nova-command">
        ...
        <article className="story-launcher"><div><span>AKTIVE MISSION</span><strong>{story.title}</strong><small className="story-category">{storyCategoryLabel(story)}</small>{story.storyType==='world_independent'&&<small className="story-presentation">{storyPresentationLabel(story,presentationWorldId)}</small>}<small>{story.subject}</small>{!mission&&<small>Mission in dieser Welt noch nicht umgesetzt</small>}</div><select aria-label="Aktive Lorestory" value={story.id} onChange={e=>selectStory(e.target.value)}>{available.map(item=><option value={item.id} key={item.id}>{item.title}</option>)}</select><button onClick={()=>setView('mission')} disabled={!mission} title={!mission?'Mission in dieser Welt noch nicht umgesetzt':undefined}><CirclePlay/> Mission vorbereiten</button></article>
      </section>
