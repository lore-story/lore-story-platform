...
import { createSupabaseLoreboardRepository, DEFAULT_LOREBOARD_STATE, MAX_ASSIGNMENT_LENGTH, MAX_ASSIGNMENT_LINES, MAX_MATERIALS } from './loreboardRepository'
...
function App() {
  ...
  const [view, setView] = useState(() => window.location.pathname === '/werkstatt' ? 'werkstatt' : initial.view)
...
function Platform(props) {
  ...
  const go=(v)=>{setView(v);setMobile(false);window.history.pushState({},'',v==='werkstatt'?'/werkstatt':'/')}
  ...
  return <div className={`platform ${platformTheme ? `world-${platformTheme.id}` : ''}`} style={themeVariables(platformTheme)}><aside className={mobile?'open':''}><div className="aside-head"><Brand light/><button className="mobile-close" onClick={()=>setMobile(false)}><X/></button></div><div className="workspace-label">MEINE LORE-WERKSTATT</div><nav>{navItems.map(([id,I,label])=><button className={view===id?'active':''} key={id} onClick={()=>go(id)}><I size={19}/>{label}{label==='Lore-Market'&&<span className="new-pill">NEU</span>}</button>)}</nav><div className="aside-world"><small>AKTIVE WELT</small><div><span>✦</span><b>{worlds.find(w=>w.id===props.activeWorld)?.name}</b></div></div><div className="account-menu"><button className="profile" aria-expanded={accountOpen} aria-controls="account-popover" onClick={()=>setAccountOpen(value=>!value)}><span>{initials}</span><div><b>{displayName}</b><small>Kontomenü öffnen</small></div><ChevronDown size={17}/></button>{accountOpen&&<div className="account-popover" id="account-popover" role="menu" aria-label="Kreativkonto"><b>Kreativkonto</b><small title={user.email}>{user.email}</small><button role="menuitem" onClick={onLogout}><LogOut size={16}/> Abmelden</button></div>}</div></aside>
    <main className="app-main"><header className="app-top"><button className="menu-btn" onClick={()=>setMobile(true)}><Menu/></button><div><span className="breadcrumb">LORE STORY /</span> {navItems.find(n=>n[0]===view)?.[2]}</div><div className="top-actions"><button className="icon-btn" aria-label="Suchen"><Search size={19}/></button></div></header><div className="view">{view==='overview'&&<Overview {...props}/>} {view==='werkstatt'&&<WorkshopApp supabase={props.supabase} user={user} onBack={()=>setView('overview')}/>} {view==='fundus'&&<Fundus {...props}/>} {view==='market'&&<Market {...props}/>} {view==='weltwechsler'&&<WorldSwitcher {...props}/>}</div></main>{toast&&<div className="toast"><Check size={17}/>{toast}</div>}</div>
}
...
function LoreboardMode({ fundus, activeStory, setActiveStory, activeWorld, setActiveWorld, setView, repository }) {
  ...
  const assignmentTooLong = assignmentDraft.length > MAX_ASSIGNMENT_LENGTH || assignmentDraft.split('\n').length > MAX_ASSIGNMENT_LINES
  ...
  const saveAssignment=()=>{if(assignmentDraft.trim()&&!assignmentTooLong){update({assignment:assignmentDraft});setDialog(null)}}
  ...
  return <main className={`loreboard-mode ${configuredTheme ? `world-${world.id}` : ''}`} style={{'--lore-accent':world.colors[0],'--lore-deep':world.colors[1],...themeVariables(configuredTheme)}}>
    ...
    {dialog==='assignment'&&<EditorDialog title="Aktuellen Auftrag bearbeiten" onClose={()=>setDialog(null)}><label className="dialog-label">Auftrag<textarea autoFocus aria-label="Aktueller Auftrag bearbeiten" rows="7" value={assignmentDraft} aria-invalid={assignmentTooLong} aria-describedby="assignment-guidance assignment-limit" onChange={e=>setAssignmentDraft(e.target.value)}/><small id="assignment-guidance">Zeilenumbrüche und Leerzeilen bleiben erhalten. Damit der Auftrag vollständig auf die Präsentationskarte passt, sind maximal {MAX_ASSIGNMENT_LINES} Zeilen und {MAX_ASSIGNMENT_LENGTH} Zeichen möglich ({assignmentDraft.split('\n').length}/{MAX_ASSIGNMENT_LINES} Zeilen · {assignmentDraft.length}/{MAX_ASSIGNMENT_LENGTH} Zeichen).</small>{assignmentTooLong&&<span id="assignment-limit" className="limit-note" role="alert">Der Auftrag ist zu lang für die Präsentationskarte. Bitte kürze ihn auf höchstens {MAX_ASSIGNMENT_LINES} Zeilen und {MAX_ASSIGNMENT_LENGTH} Zeichen.</span>}</label><div className="dialog-actions"><button onClick={()=>setDialog(null)}>Abbrechen</button><button onClick={saveAssignment} disabled={!assignmentDraft.trim()||assignmentTooLong}><Save/> Auftrag speichern</button></div></EditorDialog>}
