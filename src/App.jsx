...
import { isStoryAvailableInWorld, stories, storyCategoryLabel, worlds } from './data'
...
function LoreboardMode({ fundus, activeStory, setActiveStory, activeWorld, setActiveWorld, setView, repository }) {
  ...
  return <main className={`loreboard-mode ${configuredTheme ? `world-${world.id}` : ''}`} style={{'--lore-accent':world.colors[0],'--lore-deep':world.colors[1],...themeVariables(configuredTheme)}}>
    ...
    <section className="board-grid">
      ...
      <article className="board-widget story-launcher"><div className="widget-title"><Sparkles/><span>AKTIVE LORESTORY</span></div><div className="story-select"><span>{world.icon}</span><select aria-label="Aktive Lorestory" value={story.id} onChange={e=>selectStory(e.target.value)}>{available.map(item=><option value={item.id} key={item.id}>{item.title}</option>)}</select></div><p>{storyCategoryLabel(story)}</p><p>{story.subject} · {story.age} Jahre · {story.duration}</p><button onClick={()=>setView('mission')} disabled={!getMissionPackage(`lore-${story.worldId || story.world}-${story.id}`)} title={!getMissionPackage(`lore-${story.worldId || story.world}-${story.id}`)?'Für dieses Lernangebot ist noch keine interaktive Mission verfügbar.':undefined}><CirclePlay/> Mission vorbereiten</button></article>
    </section>
...
function WorldSwitcher({activeStory,activeWorld,setActiveWorld,setActiveStory,notify,repository}) {
  ...
  const compatible=isStoryAvailableInWorld(story,previewWorld)
  const apply=async()=>{if(!compatible||saving)return;setSaving(true);try{const board=await repository.load();await repository.save({...board,activeWorld:previewWorld,activeStory:story.id});setActiveWorld(previewWorld);setActiveStory(story.id);notify(`${worlds.find(w=>w.id===previewWorld).name} als aktive Welt gespeichert`)}catch{notify('Weltwechsel konnte nicht gespeichert werden. Es wurde nichts verändert.')}finally{setSaving(false)}}
  const preview=worlds.find(w=>w.id===previewWorld)
  return <><PageHead kicker="GLEICHE STRUKTUR. NEUE MAGIE." title="Weltwechsler" copy="Die Auswahl zeigt nur eine Vorschau. Gespeichert wird erst nach deiner Bestätigung."/><div className="switch-panel"><div className="switch-story"><span>AKTUELLE STORY</span><h3>{story.title}</h3><p>{storyCategoryLabel(story)}</p></div><div className="switch-arrow"><WandSparkles/></div><div><span className="kicker">ZIELWELT WÄHLEN</span><div className="world-options">{worlds.map(w=><button className={previewWorld===w.id?'active':''} onClick={()=>setPreviewWorld(w.id)} key={w.id} style={{'--world':w.colors[0]}}><i>{w.icon}</i><div><b>{w.name}</b><small>{w.label}</small></div>{previewWorld===w.id&&<Check/>}</button>)}</div></div></div><div className="transfer-preview"><div><span>AKTUELL GESPEICHERT</span><h3>{worlds.find(w=>w.id===activeWorld).name}</h3></div><ArrowRight/><div><span>VORSCHAU</span><h3>{preview.name}</h3><p>{compatible?(story.variants?.[previewWorld]||story.title):`${story.title} ist nur in ${worlds.find(w=>w.id===(story.worldId||story.world)).name} spielbar.`}</p></div></div><button className="btn gold switch-submit" disabled={!compatible||saving||previewWorld===activeWorld} title={!compatible?'Weltgebundene Storys können nicht in einer anderen Welt aktiviert werden.':previewWorld===activeWorld?'Diese Welt ist bereits aktiv.':undefined} onClick={apply}><Sparkles size={17}/> {saving?'Wird gespeichert …':'Weltwechsel anwenden'}</button></>
}
