...
function Platform(props) {
  ...
  if(view==='mission') { const story=stories.find(item=>item.id===props.activeStory); const world=worlds.find(item=>item.id===props.activeWorld); const mission=story?.world===world?.id ? getMissionPackage(`lore-${story.world}-${story.id}`) : null; return <MissionLobby supabase={props.supabase} user={user} world={world} theme={getWorldTheme(world?.id)} mission={mission} onBack={()=>setView('loreboard')}/> }
