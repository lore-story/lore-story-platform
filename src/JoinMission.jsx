...
export default function JoinMission({ supabase, code }) {
  ...
  if (teacherBrowser || screen === 'teacher') return <main className={`${themedPage} mission-join-page`} style={themeVariables(theme)}><div className="join-card"><Rocket/><h1>Schülerzugang</h1><p>Dieser Browser ist als Lehrkraft angemeldet. Öffne den Schülerzugang auf einem Schülergerät oder in einem privaten Browserfenster.</p><a className="join-back" href="/"><ArrowLeft/> Zurück zum Loreboard</a></div></main>
  if (screen === 'loading') return <main className={`${themedPage} mission-join-page`} style={themeVariables(theme)}><div className="join-card"><Rocket/><h1>Mission wird gesucht …</h1></div></main>
  if (screen === 'error' || screen === 'removed') return <main className={`${themedPage} mission-join-page`} style={themeVariables(theme)}><div className="join-card removed-card"><WifiOff/><h1>{screen === 'removed' ? 'Du wurdest aus der Crew entfernt' : 'Zugang nicht möglich'}</h1><p role="alert">{message}</p>{screen === 'error' ? <button onClick={() => inspect()}>Erneut versuchen</button> : <button className="primary" onClick={retryAfterRemoval}><RefreshCw/> Erneut beitreten</button>}</div></main>
  if (screen === 'choose') { const blocked=joinBlockedReason(info,chosen); return <main className={`${themedPage} mission-join-page student-join-page`} style={themeVariables(theme)}><div className="join-card wide"><Rocket/><small>LORE {theme.name.toUpperCase()}</small><h1>{info.title}</h1><p>Wähle dein anonymes Rufzeichen. Es werden keine Namen benötigt.</p><div className="callsign-grid" role="group" aria-label="Rufzeichen auswählen">{info.callsigns.map(name => <button type="button" key={name} disabled={info.taken_callsigns.includes(name)} aria-pressed={chosen === name} aria-label={`${name}${info.taken_callsigns.includes(name) ? ', belegt' : chosen === name ? ', ausgewählt' : ', frei'}`} className={`${chosen === name ? 'selected' : ''} ${info.taken_callsigns.includes(name) ? 'taken' : 'available'}`} onClick={() => setChosen(name)}><span>{name}</span>{chosen === name ? <small className="selection-mark"><Check/> Ausgewählt</small> : info.taken_callsigns.includes(name) ? <small>Belegt</small> : <small>Frei</small>}</button>)}</div>{message && <p role="alert">{message}</p>}<div className="join-confirmation">{blocked&&<p className="join-block-reason" role="status">{blocked}</p>}<button className="primary" disabled={Boolean(blocked)} title={blocked||undefined} onClick={() => join(chosen)}>Mit {chosen || 'Rufzeichen'} beitreten</button></div></div></main> }
