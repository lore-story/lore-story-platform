...
export default function JoinMission({ supabase, code }) {
  ...
  return <main className={themedPage} style={themeVariables(theme)}><section className="student-runtime"><header className="student-runtime-header"><div className={`connection ${online ? 'online' : ''}`}>{online ? <Wifi/> : <WifiOff/>}{online ? 'Crew-Netzwerk verbunden' : 'Verbindung unterbrochen'}</div><span className="student-access-state">{info.joining_open?'Zugang geöffnet':'Zugang geschlossen'}</span><button aria-label="Vollbild öffnen" onClick={()=>document.documentElement.requestFullscreen?.()}><Expand/></button></header><div><small>RUFZEICHEN · {chosen}</small><h1>{completed ? 'Mission abgeschlossen' : paused ? 'Übertragung pausiert' : info.finale_status==='finished'?'Fortsetzung folgt':scene.title}</h1><h2>{info.title}</h2></div>{finaleRemaining!==null&&finaleRemaining>0?<div className="student-finale-countdown" aria-live="assertive">{finaleRemaining}</div>:<div className="student-runtime-content">{completed ? <p>Danke für deinen Einsatz. Die {theme.name}-Crew meldet sich wieder.</p> : paused ? <p>NOVA: Bleib an deinem Platz. Die Lehrkraft setzt die Übertragung fort.</p> : active ? <><div className="student-nova"><b>NOVA</b><p>{scene.message}</p></div><section className="student-task"><small>AKTUELLER AUFTRAG</small><strong>{scene.task}</strong></section></> : <p>NOVA: Warte bitte, bis die Lehrkraft die Mission startet.</p>}</div>}{scene.ready&&active&&finaleRemaining===null&&<button className="primary" onClick={() => ready(!info.ready_scene_id)}>{info.ready_scene_id === info.current_scene_id ? <><Check/> Bereitschaft gemeldet</> : <><Radio/> Ich bin bereit</>}</button>}{message && <p role="alert">{message}</p>}</section></main>
}
