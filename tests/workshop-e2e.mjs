...
async function layoutChecks(page,width,height,name){
 ...
 assert.equal(metrics.pageScroll,false,`${name}: whole page must not scroll`);assert.equal(metrics.editor.h,height);assert.ok(metrics.workspace>width*(width<=1024?.62:.35),`${name}: workspace is too narrow`);if(width>1200){assert.equal(metrics.railScroll,false);assert.equal(metrics.propsScroll,false);assert.ok(metrics.scrollables.length<=1,`${name}: nested scrolling: ${metrics.scrollables}`)}const clipped=await page.locator('.scene-workspace input,.scene-workspace textarea,.scene-workspace button').evaluateAll(nodes=>nodes.filter(el=>{const r=el.getBoundingClientRect(),host=el.closest('.scene-preview').getBoundingClientRect();return r.bottom>host.bottom+1||r.right>host.right+1}).length);assert.equal(clipped,0,`${name}: editor controls are clipped`)
...
try{
 for(const [w,h,n] of [[1920,1080,'1920x1080'],[1440,1000,'1440x1000'],[1024,1366,'1024x1366']]){const p=await browser.newPage({viewport:{width:w,height:h}});await layoutChecks(p,w,h,n);await p.close()}
 ...
 const preview=page.getByTestId('scene-preview');assert.equal(await preview.locator('img[alt="Leuchtendes Archiv"]').count(),1)
 ...
 await page.locator('.desktop-rail .scene-select').nth(1).click();await page.getByText('Bild konnte nicht geladen werden').waitFor()
 ...
 await page.getByRole('button',{name:'Bild aus Szene entfernen'}).click();await page.waitForTimeout(900);assert.equal(await page.evaluate(()=>window.__workshop.scenes[0].content.media.path),undefined)
 await page.locator('.desktop-rail .scene-select').nth(2).click();await page.waitForTimeout(900);const before=await page.evaluate(()=>({scenes:JSON.stringify(window.__workshop.scenes),saves:window.__workshop.saveCount,sessions:window.__workshop.sessionWrites}));await page.getByRole('button',{name:'Mission testen'}).click();assert.match(await page.getByText(/Szene 1 \/ 3/).innerText(),/Szene 1/);assert.equal(await page.getByText('Ankunft').count(),1)
