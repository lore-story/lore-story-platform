...
try{
 ...
 for(const [typeName,kind] of [['Erzählung','narrative'],['Lernauftrag','assignment'],['Übergang','transition']]){
  ...
  for(const [layoutName,layout] of [['Fokus: Text','text'],['Geteilt: Text und Medium','split'],['Fokus: Medium','media']]){await page.locator('.layout-selector').getByRole('button',{name:new RegExp(layoutName)}).click();const boxes=await preview.evaluate(el=>{const copy=el.querySelector('.scene-copy').getBoundingClientRect(),media=el.querySelector('figure').getBoundingClientRect(),style=getComputedStyle(el);return{layout:el.dataset.layout,copy:copy.width,media:media.width,color:style.color,background:style.backgroundColor}});if(layout==='text')assert.ok(boxes.copy>boxes.media*1.7,`${kind} text focus ratio`);if(layout==='split')assert.ok(Math.abs(boxes.copy-boxes.media)<5,`${kind} split ratio`);if(layout==='media')assert.ok(boxes.media>boxes.copy*1.8,`${kind} media focus ratio ${JSON.stringify(boxes)}`);if(kind==='assignment'){assert.notEqual(boxes.background,'rgb(248, 251, 255)');assert.notEqual(boxes.color,'rgb(22, 41, 58)')}await page.screenshot({path:`artifacts/workshop-${kind}-${layout}.png`,fullPage:false})}
  if(kind==='transition')assert.equal(await page.getByText('Bereit machen',{exact:true}).count(),1)
 }
 ...
 await page.getByRole('button',{name:/Weiter/}).click();assert.match(await page.getByText(/Szene 2 \/ 3/).innerText(),/Szene 2/);await page.getByRole('button',{name:/Zurück/}).last().click();assert.match(await page.getByText(/Szene 1 \/ 3/).innerText(),/Szene 1/);await page.getByRole('button',{name:/Weiter/}).click();await page.getByRole('button',{name:/Weiter/}).click();assert.equal(await page.locator('.scene-continue').isDisabled(),true);await page.getByRole('button',{name:'Wir sind bereit'}).click();await page.getByRole('button',{name:'Bereit ✓'}).waitFor();assert.equal(await page.locator('.scene-continue').isEnabled(),true);await page.locator('.scene-continue').click();await page.getByRole('heading',{name:'Testlauf beendet'}).waitFor();assert.equal(await page.evaluate(()=>window.__workshop.sessionWrites),before.sessions);assert.equal(await page.evaluate(()=>JSON.stringify(window.__workshop.scenes)),before.scenes);assert.equal(await page.evaluate(()=>window.__workshop.saveCount),before.saves);await page.getByRole('button',{name:'Zurück zum Editor'}).click();await page.getByLabel('Missionsname').waitFor()
