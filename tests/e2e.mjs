...
try {
  ...
  assert.equal(await desktop.evaluate(() => JSON.parse(localStorage.getItem('e2e-cloud-row')).state.activeWorld), 'astra')
  ...
  await desktop.getByText('Story erfolgreich nach Astra übertragen').waitFor()
