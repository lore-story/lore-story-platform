...
const backend = { version: 0, runs: [], participants: [], nextRun: 1, nextParticipant: 1 }
...
try {
  ...
  teacher.once('dialog', dialog => dialog.accept()); await teacher.getByRole('button', { name: 'Astrofuchs entfernen' }).click(); await studentOne.getByText('Du wurdest aus dieser Mission entfernt').waitFor(); await studentOne.reload(); await studentOne.getByText('Du wurdest aus dieser Mission entfernt').waitFor()
  teacher.once('dialog', dialog => dialog.accept()); await teacher.getByRole('button', { name: 'Mission abschließen' }).click(); await studentTwo.getByText('Mission ist abgeschlossen').waitFor(); assert.equal(rpc('student', 'student-2', 'update_my_mission_presence', { p_session_id: 'run-1', p_connected: true, p_ready: true }).error.message, 'MISSION_COMPLETED')
