...
begin
 ...
 if not v_participant_found or v_participant.status='removed' then
 ...
 begin
   if v_participant_found then
     update public.mission_participants mp set callsign=v_callsign,status='connected',ready_scene_id=null,last_seen_at=pg_catalog.now(),removed_at=null,joined_at=case when mp.status='removed' then pg_catalog.now() else mp.joined_at end where mp.id=v_participant.id returning mp.* into v_participant;
   else
